...
def test_get_agents_for_selection():
    ...
    with patch('gradio_debate_app.make_api_request') as mock_request:
        ...
        assert len(result) == 2, f"期望 2 個選項，實際得到 {len(result)}"
        assert "測試Agent1 (analyst)" in result[0], f"第一個選項格式不正確: {result[0]}"
...
def test_load_initial_data():
    ...
    with patch('gradio_debate_app.make_api_request') as mock_request:
        ...
        # 驗證結果
        assert len(result) == 2, f"期望返回 2 個更新，實際得到 {len(result)}"

        choices_update, count_update = result

        # 验证选择更新
        assert hasattr(choices_update, 'choices'), "第一個傳回值應該是 gr.update 物件"
        assert len(choices_update.choices) == 1, f"期望 1 個選擇，實際得到 {len(choices_update.choices)}"
...
def test_empty_agents():
    ...
    with patch('gradio_debate_app.make_api_request') as mock_request:
        ...
        result = get_agents_for_selection()
        assert len(result) == 0, f"期望空列表，實際得到 {len(result)} 個選項"

        # 测试 load_initial_data
        result = load_initial_data()
        choices_update, count_update = result
        assert len(choices_update.choices) == 0, f"期望空選擇列表，實際得到 {len(choices_update.choices)}"
        assert count_update.value == "當前 Agent 總數：0", f"計數文本不正確: {count_update.value}"
