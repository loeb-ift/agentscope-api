...
请确保你的分析专业、客观、全面，并基于实际辩论内容，提供具体的数据和逻辑支持。"""
        ...
        try:
            ...
            try:
                ...
                try:
                    # 移除可能的BOM标记和所有空白字符
                    conclusion_text = conclusion_text.lstrip('\ufeff').strip()
                    
                    # 移除开头的换行符和空白
                    while conclusion_text.startswith(('\n', '\r', ' ', '\t')):
                    ...
                    try:
                        ...
                        try:
                            ...
                            conclusion_text = re.sub(r'\r\s*', ' ', conclusion_text)
                            conclusion_data = json.loads(conclusion_text)
                        except json.JSONDecodeError:
                            # 如果仍然失败，手动构建结论数据
                            print(f"JSON解析错误: {e}, 原始文本: {conclusion_text[:200]}...")
                            conclusion_data = {
